...
import com.devonfw.module.odata.logic.util.ODataResourcesClassUtil;

/**
 * Provider for the entity data model used in the reference scenario
 */
public class ODataEdmProviderExtension extends EdmProvider {
  ...
  private static Map<String, ODataImportFunctionHolder> functionImportHolders = null;

  public static Map<String, ODataImportFunctionHolder> getFunctionImportHolders() {
   ...
   */
  public ODataEdmProviderExtension(final Collection<Class<?>> annotatedClasses,
    Map<String, ODataImportFunctionHolder> functionImportHolders) throws ODataException {

    ODataEdmProviderExtension.functionImportHolders = functionImportHolders;
    this.annotatedClasses = new ArrayList<>(annotatedClasses.size());
    this.annotatedClasses
      .addAll(annotatedClasses.stream().filter(clazz -> ODataAnnotationUtil.isEdmAnnotated(clazz))
  ...
  private SchemaBuilder getSchemaBuilder(final String namespace, final Class<?> aClass) {
    ...
    if (usedNamespace.isEmpty()) {
      usedNamespace = ODataAnnotationUtil.getCanonicalNamespace(aClass);
    }
  ...
  private void handleEntityContainer(final Class<?> aClass) {
    ...
    if (entityType != null) {
      ...
      String containerName = ODataAnnotationUtil.extractContainerName(aClass);
    ...
    for (Method method : methods) {
      ...
      if (functionImport != null) {
        ...
        ContainerBuilder builder = this.containerName2ContainerBuilder.get(containerName);
        if (builder == null) {
          String namespace = ODataAnnotationUtil.generateNamespace(aClass);
  ...
  private FunctionImport createFunctionImport(final Method method) {
    ...
    String entitySetName = ODataAnnotationUtil.extractEntitySetName(method);
    String httpMethod = ODataAnnotationUtil.extractHttpMethod(method);
    String functionImportName = ODataAnnotationUtil.extractFunctionImportName(method);
    ArrayList<FunctionImportParameter> functionImportParameters =
      ODataAnnotationUtil.extractFunctionImportParameters(method);
    ReturnType returnType = ODataAnnotationUtil.extractReturnType(method);
  ...
  private EntitySet createEntitySet(final FullQualifiedName typeName, final Class<?> entitySetClass) {
    ...
    String entitySetName = ODataAnnotationUtil.extractEntitySetName(entitySetClass);
  ...
  private FullQualifiedName createFqnForEntityType(final Class<?> annotatedClass) {
    ...
    return ODataAnnotationUtil.extractEntityTypeFqn(annotatedClass);
  }
  ...
  static class TypeBuilder {
    ...
    @EdmProperty final private String namespace;
    ...
    public static TypeBuilder init(final EdmEntityType entity, final Class<?> aClass) {
      ...
      return new TypeBuilder(ODataAnnotationUtil.extractEntityTypeFqn(entity, aClass)).withClass(aClass);
    }

    public static TypeBuilder init(final EdmComplexType entity, final Class<?> aClass) {

      return new TypeBuilder(ODataAnnotationUtil.extractComplexTypeFqn(entity, aClass)).withClass(aClass);
    }
    ...
    private TypeBuilder withClass(final Class<?> aClass) {
      ...
      for (Field field : fields) {
        ...
        if (enp != null) {
          ...
          Class<?> toClass = ODataResourcesClassUtil.getFieldType(field);
          ODataAnnotationNavInfoUtil info =
            ODataAnnotationUtil.getCommonNavigationInfo(fromClass, toClass, field.getName());
    ...
    private Property createSimpleProperty(final EdmProperty ep, final Field field) {
      ...
      String entityName = ODataAnnotationUtil.getPropertyName(field);
      sp.setName(entityName);
      //
      EdmType type = ep.type();
      if (type == EdmType.NULL) {
        type = ODataAnnotationUtil.mapType(field.getType());
      }
      sp.setType(ODataAnnotationUtil.mapTypeKind(type));
    ...
    private Property createComplexProperty(final Field field, final String defaultNamespace) {
      ...
      cp.setType(
        new FullQualifiedName(complexEntityNamespace,
          ODataAnnotationUtil.extractComplexTypeName(field.getType())));
      return cp;
    }

    private NavigationProperty createNavigationProperty(final String namespace, Field field,
      ODataAnnotationNavInfoUtil navInfo) {

      NavigationProperty navProp = new NavigationProperty();
      navProp.setName(ODataAnnotationUtil.getPropertyName(field));
    ...
    private FullQualifiedName createBaseEntityFqn(final Class<?> aClass) {
      ...
      return ODataAnnotationUtil.extractEntityTypeFqn(baseEntityClass);
    }

    private Association createAssociation(final ODataAnnotationNavInfoUtil info) {
    ...
    private String getCanonicalName(final Field field) {
      ...
      return ODataAnnotationUtil.getCanonicalName(field);
    }

    private boolean isAnnotatedEntity(final Class<?> clazz) {

      return ODataAnnotationUtil.isEdmTypeAnnotated(clazz);
    }
  }
